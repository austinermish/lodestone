...
Status: Implemented  
Owner: Lodestone  
Scope: Server update lifecycle for Deploy-to-Cloudflare Lodestone servers

## Problem: the Day 2 trap

Lodestone uses Cloudflare's Deploy button to optimize for a 60-second setup. That flow works well for onboarding, but creates a lifecycle problem:
...
Because workflows are stripped during deploy clone, Lodestone bootstraps them using a GitHub deep-link:

- Plugin collects the generated repo URL.
- Plugin opens a pre-filled GitHub file creation URL for `.github/workflows/lodestone-ops.yml`.
...
`lodestone-ops.yml` is intentionally small and dispatch-only. It calls a reusable workflow hosted in the upstream Lodestone repo:
...
Lodestone updates the server by applying a release artifact (`lodestone-server.zip`) into the generated deployment repo, committing, and pushing. Cloudflare redeploys from that commit.
...
Updater reads `lodestone-server-manifest.json`. If `migrationRequired: true`, automatic update aborts with a clear error and manual migration instruction.
...
Deploy is an install primitive, not an in-place update primitive. Re-deploy can create a new project path and risks orphaning user state if misused. Lodestone update execution therefore happens at the Git layer.
